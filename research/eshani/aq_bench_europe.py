...
# filters data to only include European data stations
lon = np.array(data['lon'])
lat = np.array(data['lat'])
mask_lon = (lon>=-25) & (lon<=65) 
mask_lat = (lat>=35) & (lat<=72)
...
for train_index, test_index in kfold.split(x):
    ...
    rf_predictor = RandomForestRegressor(max_depth=None, min_samples_split=5, min_samples_leaf=1, min_weight_fraction_leaf=0.0, max_leaf_nodes=None, min_impurity_decrease=0.0, bootstrap=True, oob_score=True, n_jobs=None, random_state=12, verbose=0, warm_start=False, ccp_alpha=0.0, max_samples=None)
...
plt.title('Predicted Ozone vs Actual Ozone in Europe', fontsize=12)
...
plt.title('Feature Importance vs Permutation Importance of Europe', fontsize=12)
...
plt.savefig('importance_europe.png')
plt.show()
