...
month = 'jul'
root_dir = '/Users/kelseydoerksen/exp_runs/rf/{}/all_gee_added'.format(month)
#gee_dir = '/Users/kelseydoerksen/gee'
gee_dir = '/Users/kelseydoerksen/exp_runs/rf/{}/all_gee_added'.format(month)
...
for year in years:
    ...
    modis = xr.open_dataset('{}/{}/{}_modis_lc_no_time.nc'.format(gee_dir, year, year))
    ...
    momo = xr.open_dataset('{}/{}/test.data.nc'.format(root_dir, year))
    ...
    formatted_modis.to_netcdf('{}/{}/test.modis.nc'.format(root_dir, year))
...
for k, v in gee_type.items():
    train_2011 = v[1].merge(v[2]).merge(v[3]).merge(v[4])
    train_2011.to_netcdf('{}/2011/train.{}.nc'.format(root_dir, k))

    train_2012 = v[0].merge(v[2]).merge(v[3]).merge(v[4])
    train_2012.to_netcdf('{}/2012/train.{}.nc'.format(root_dir,k))

    train_2013 = v[0].merge(v[1]).merge(v[3]).merge(v[4])
    train_2013.to_netcdf('{}/2013/train.{}.nc'.format(root_dir, k))

    train_2014 = v[0].merge(v[1]).merge(v[2]).merge(v[4])
    train_2014.to_netcdf('{}/2014/train.{}.nc'.format(root_dir, k))

    train_2015 = v[0].merge(v[1]).merge(v[2]).merge(v[3])
    train_2015.to_netcdf('{}/2015/train.{}.nc'.format(root_dir, k))
