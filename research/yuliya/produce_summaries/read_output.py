...
def load_importances(summaries_dir):
    ...
    for i in range(mi.shape[1]):
        ...
        mi_sorted[:, i] = mi[mi_idx, i] / np.nanmax(mi[mi_idx, i])
        
        pi_idx = np.hstack([np.where(pi_names[:, i] == label)[0] 
                            for label in reference_labels])
        pi_sorted[:, i] = pi[pi_idx, i] / np.nanmax(pi[pi_idx, i])
...
def load_contributions(summaries_dir, reference_labels):
    ...
    for m in range(len(plots.MONTHS)):
        ...
        if mask_month.sum() > 0:
            ...
            cont_abs = np.abs((data.to_array().values))
            mean_mon = np.nanmean(cont_abs, axis = (1,2))
            #cont_abs_mean.append(mean_mon)
            #std_mon = np.nanstd(np.abs((data.to_array().values)), axis = (1,2))
            #cont_abs_std.append(std_mon)
            #cont_names.append(np.hstack(list(data.keys())))
            
            labels = np.hstack(list(data.keys()))
            cont_norm_mean = mean_mon / mean_mon.max()
            #cont_norm_std = std_mon / std_mon.max()
            
            cont_idx = np.hstack([np.where(labels == r)[0] for r in reference_labels])
            if len(cont_idx) > 0:
                cont_sorted_mean.append(cont_norm_mean[cont_idx] / cont_norm_mean[cont_idx].max())
                #cont_sorted_std.append(cont_norm_std[cont_idx] / cont_norm_std[cont_idx].max())
            else:
                cont_sorted_mean.append(np.repeat(np.nan, P))
                #cont_sorted_std.append(np.repeat(np.nan, len(reference_labels)))
            
            
        else:
            cont_sorted_mean.append(np.repeat(np.nan, P))
            #cont_sorted_std[:, m] = np.repeat(np.nan, len(reference_labels))
            #cont_names.append(np.repeat(np.nan, len(reference_labels)))
    
    cont_sorted_mean = np.column_stack(cont_sorted_mean)
    #set std to zero since not saved from the random forest
    #cont_sorted_std = np.column_stack(cont_sorted_std)
    cont_sorted_std = np.zeros_like(cont_sorted_mean)
    
    #process contributions
    # cont_norm = [a / np.nanmax(a) for a in cont_abs_mean]
    # cont_norm_std = [a / np.nanmax(a) for a in cont_abs_std]
    # cont_sorted = np.zeros((len(reference_labels), len(cont_norm)))
    # cont_sorted_std = np.zeros((len(reference_labels), len(cont_norm)))
    ...
    #         cont_sorted_std[:, i] = cont_norm_std[i][cont_idx] / cont_norm_std[i][cont_idx].max()
    #     else:
    #         cont_sorted[:, i] = np.nan 
    #         cont_sorted_std[:, i] = np.nan 

    
    contributions = {'monthly_mean': cont_sorted_mean, 
                     'monthly_std': cont_sorted_std,
                     'mean': np.nanmean(cont_sorted_mean, axis = 1),
                     'std': np.nanstd(cont_sorted_mean, axis = 1)}
