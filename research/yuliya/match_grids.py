...
for year in years:
    ...
    for month in tqdm(months):
        ...
        for d in days:
            ...
            momo_dat.append(momo['data'][mask_d, :, :][mask_h, :, :].mean(axis = 0))
    ...
    # new_file = f'{data_root_dir}/processed/summary_dp/momo_matched_{year}_{month}.h5' 
    # with closing(h5py.File(new_file, 'w')) as f:
    #     for s in summaries:
    #         f['toar'][s] = np.dstack(toar_to_momo[s])
    #     f['momo'] = momo_dat
    #     f['date'] = momo_ud
    ...
    for d in range(bias.shape[2]):
        ...
        plt.savefig(f'{data_root_dir}/processed/plots/bias_{year}_{month}_{d}.png', 
                    bbox_inches = 'tight')
        plt.close()
    ...
    ax.set_extent([-140, -50, 10, 80], crs=ccrs.PlateCarree())
    plt.savefig(f'{data_root_dir}/processed/plots/bias_{year}_{month}_mean.png', 
                bbox_inches = 'tight')
    plt.close()
